...
#ifndef GMATELASTOPLASTICFINITESTRAINSIMO_CARTESIAN3D_ARRAY_HPP
#define GMATELASTOPLASTICFINITESTRAINSIMO_CARTESIAN3D_ARRAY_HPP
...
{
    GMATELASTOPLASTICFINITESTRAINSIMO_ASSERT(xt::has_shape(ret, m_shape));
...
{
    GMATELASTOPLASTICFINITESTRAINSIMO_ASSERT(xt::has_shape(m_type, I.shape()));
    ...
    const xt::xtensor<double, 1>& G)
{
    ...
    GMATELASTOPLASTICFINITESTRAINSIMO_ASSERT(xt::has_shape(m_type, I.shape()));
    GMATELASTOPLASTICFINITESTRAINSIMO_ASSERT(xt::has_shape(m_type, idx.shape()));
    GMATELASTOPLASTICFINITESTRAINSIMO_ASSERT(xt::all(xt::equal(I, 0ul) || xt::equal(I, 1ul)));
    GMATELASTOPLASTICFINITESTRAINSIMO_ASSERT(
        xt::all(xt::equal(xt::where(xt::equal(I, 1ul), m_type, Type::Unset), Type::Unset)));
    ...
    const xt::xtensor<double, 1>& H)
{
    GMATELASTOPLASTICFINITESTRAINSIMO_ASSERT(xt::amax(idx)[0] == K.size() - 1);
    GMATELASTOPLASTICFINITESTRAINSIMO_ASSERT(K.size() == G.size());
    GMATELASTOPLASTICFINITESTRAINSIMO_ASSERT(K.size() == sigy0.size());
...
template <size_t N>
inline void Array<N>::setDefGrad(const xt::xtensor<double, N + 2>& arg, bool tangent)
{
    ...
    for (size_t i = 0; i < m_size; ++i) {
        ...
        case Type::Unset:
            break;
        case Type::Elastic:
            m_Elastic[m_index.data()[i]].setDefGradPtr(&arg.data()[i * m_stride_tensor2], tangent);
            break;
        case Type::LinearHardening:
            m_LinearHardening[m_index.data()[i]].setDefGradPtr(&arg.data()[i * m_stride_tensor2], tangent);
            break;
        }
    }
}
...
template <size_t N>
inline void Array<N>::defGrad(xt::xtensor<double, N + 2>& ret) const
{
    GMATELASTOPLASTICFINITESTRAINSIMO_ASSERT(xt::has_shape(ret, m_shape_tensor2));
    ...
    for (size_t i = 0; i < m_size; ++i) {
        ...
        case Type::Unset:
            ...
            break;
        case Type::Elastic:
            m_Elastic[m_index.data()[i]].defGradPtr(&ret.data()[i * m_stride_tensor2]);
            break;
        case Type::LinearHardening:
            m_LinearHardening[m_index.data()[i]].defGradPtr(&ret.data()[i * m_stride_tensor2]);
            break;
        }
    }
}
...
{
    GMATELASTOPLASTICFINITESTRAINSIMO_ASSERT(xt::has_shape(ret, m_shape_tensor4));
...
template <size_t N>
inline xt::xtensor<double, N + 2> Array<N>::DefGrad() const
{
...
{
    GMATELASTOPLASTICFINITESTRAINSIMO_ASSERT(m_type[index] == Type::Elastic);
...
{
    GMATELASTOPLASTICFINITESTRAINSIMO_ASSERT(m_type[index] == Type::LinearHardening);
